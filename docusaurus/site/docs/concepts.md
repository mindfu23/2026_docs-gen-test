...
The Open5e v2 data model, anchored to the spec.
...
List endpoints return a page-number envelope (`count`, `next`, `previous`, `results`). Control
it with **`page`** and **`limit`**; follow `next` until `null`.
...
Content is grouped into **`documents`** (source books/SRDs), each tied to a `license`,
